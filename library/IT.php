...
class HTML_Template_IT {
    ...
    function setVariable($variable, $value = "") {
         //---actualizacion 22-11-2004 (Diego - Marcelo)
			if(strstr($value,"$")){
				if(strlen(trim(substr($value,strpos($value,"$"),2)))>1 && strlen(trim(substr($value,(strpos($value,"$")-1),1)))==0){
     ...
     if ($this->variableCache[$variable]) {
         
     }//die($this->variableCache[$variable]);
