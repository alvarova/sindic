...
$tipoafiliado="";
if ($_POST['filtro']==2) {
	$tipoafiliado="(`afiliados`.sindicato = 1)  AND ";
} else if ($_POST['filtro']==3) {
	$tipoafiliado="(`afiliados`.sindicato = 0)  AND ";
}

$consulta = "SELECT `afiliados`.id_afiliado, `afiliados`.nombre, `afiliados`.id_farmacia, `afiliados`.fecha_ingreso, `farmacias`.localidad, `farmacias`.razon_social, `afiliados`.sindicato 
					FROM  afiliados    LEFT JOIN farmacias	ON  `afiliados`.id_farmacia =`farmacias`.id_farmacia
 					WHERE  ".$tipoafiliado." (`afiliados`.fecha_baja =  '0000-00-00')  ";
...
while (!$rs->EOF) {
				...
				$body->setCurrentBlock("listado");
					...
					$body->setVariable("fechaalta", $rs->fields['3']);
					$body->setVariable("tipoafiliado", $rs->fields['6']);
					if ($rs->fields['6']=='1') {
							$afi++;
							$body->setVariable("nrotipoafiliado", $afi);
					}else{
							$nafi++;
							$body->setVariable("nrotipoafiliado", $nafi);
					}
