...
$rs2 = $db2->Connect("$dbhost", "$dbuname", "", "$dbname");
$consulta2 = "SELECT * FROM afiliados_crc Order By id_afiliado";
...
$cc = count($arr_cat);
for ($i=0; $i<$cc; $i++){
 	$key=substr($arr_cat[$i][1],0,3);
 	$arrcat[$key]=$arr_cat[$i][0];
}
...
$table = new XBaseTable("../afiliados.DBF");
$table->open();
	    ...
	    foreach ($table->getColumns() as $i=>$c) {
			...
			if ($reg_found_bandera) { 
				...
				if ($col==2) {
				        ...
				        $cat=$arrcat[substr($columna_dbf,0,3)];
